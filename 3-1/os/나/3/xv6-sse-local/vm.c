...
int munmap(void *addr, int length)
{
  ...
  if(add <= m2[k[0]].addr){
	if(add + length < m2[k[t]].addr + m2[k[t]].length){
		...
		for(j=m2[k[0]].addr; j< m2[k[t]].addr + m2[k[t]].length; j+=PGSIZE){
			pte = walkpgdir(myproc()->pgdir, (void *)j, 0);	
	...
	if(add + length < m2[k[t]].addr + m2[k[t]].length){
		for(j=add; j< add + length; j+= PGSIZE){
			pte = walkpgdir(myproc()->pgdir, (void *)j, 0);
		...
		j=0;
		while(m2[k[j]].addr> add){
			if(j ==t)
				break;
			j++;
			}
		while(m2[k[j]].addr+ m2[k[j]].length <add + length){
			if(j ==t)
				break;
			m2[k[j]].addr = 0;
			m2[k[j]].length = 0;
		...
		for(j=add; j< m2[k[t]].addr + m2[k[t]].length; j+=PGSIZE){
			pte = walkpgdir(myproc()->pgdir, (void *)j, 0);
